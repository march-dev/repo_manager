...
class ProjectSizeBar extends StatelessObserverWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // ListView.builder/.separated handles by discarding and recreating row
    // widgets rather than moving them — would otherwise lose track of it
    // entirely: a fresh widget has no "previous build" to animate from, so
    // without this it either replays the from-zero entrance animation for
    // data that isn't new, or (if that's suppressed some other way) pops
    // in unanimated even when the data genuinely did just change.
    // Remembering the actual last-shown values lets a fresh widget
    // continue the transition from wherever it visually left off, whether
    // or not the underlying Element survived.
    final previous = item.lastShownSize;
    if (size != null) item.lastShownSize = size;
...
class _LoadingSizeBar extends StatelessWidget {
  const _LoadingSizeBar();

  static const _outerHeight = ProjectSizeBar.height + SizeBar.framePadding * 2;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: LinearProgressIndicator(
        ...
        // A more app-friendly color than the theme's default accent; falls
        // back to the core segment color used throughout the bar.
...
class _LoadedSizeBar extends StatelessWidget {
  const _LoadedSizeBar({
    required this.size,
  ...
  // Below this, the core and cache dots (each ProjectSizeBar.height wide,
  // plus the gap between them) don't both fit and end up overlapping
  // instead of rendering as two distinct circles — so no bar is ever
  // scaled narrower than the width two full-size dots actually need. This
  // is the *outer* width passed to SizeBar, so it also has to cover the
  // frame padding SizeBar reserves on each side for its own border —
  // otherwise the segments themselves would still be squeezed below the
  // space they need.
  static const _minBarWidth =
      2 * ProjectSizeBar.height + SizeBar.gap + 2 * SizeBar.framePadding;

  final ProjectSizeModel size;
  final ProjectSizeModel? previous;

  // The largest project total currently in the list — see
  // ProjectSizeBar.maxTotalBytes.
  final int maxTotalBytes;
